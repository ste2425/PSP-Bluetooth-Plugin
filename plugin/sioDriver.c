...
#define SERIAL_BUFFER_SIZE 			20				// Unlikely to require a larger buffer, PSP is limited to 4800 bytes per second anyway
...
void pspUARTWaitForData(unsigned int timeout)
{
   unsigned int k1 = pspSdkSetK1(0);
	...
	sceKernelWaitEventFlag(sio_eventflag, SIO_CHAR_RECV_EVENT, PSP_EVENT_WAITAND|PSP_EVENT_WAITCLEAR, &result, &timeout); //timeout could be null=forever
